import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
from datetime import datetime, time
import pytz

st.set_page_config(layout="wide")
st.title("Scanner – Setup Roberson (Diário + Semanal + Estatística)")

# =========================================================
# LISTA FIXA DE ATIVOS
# =========================================================

ativos_scan = sorted(set([
    "RRRP3.SA","ALOS3.SA","ALPA4.SA","ABEV3.SA","ARZZ3.SA","ASAI3.SA","AZUL4.SA","B3SA3.SA","BBAS3.SA","BBDC3.SA",
    "BBDC4.SA","BBSE3.SA","BEEF3.SA","BPAC11.SA","BRAP4.SA","BRFS3.SA","BRKM5.SA","CCRO3.SA","CMIG4.SA","CMIN3.SA",
    "COGN3.SA","CPFE3.SA","CPLE6.SA","CRFB3.SA","CSAN3.SA","CSNA3.SA","CYRE3.SA","DXCO3.SA","EGIE3.SA","ELET3.SA",
    "ELET6.SA","EMBR3.SA","ENEV3.SA","ENGI11.SA","EQTL3.SA","EZTC3.SA","FLRY3.SA","GGBR4.SA","GOAU4.SA","GOLL4.SA",
    "HAPV3.SA","HYPE3.SA","ITSA4.SA","ITUB4.SA","JBSS3.SA","KLBN11.SA","LREN3.SA","LWSA3.SA","MGLU3.SA","MRFG3.SA",
    "MRVE3.SA","MULT3.SA","NTCO3.SA","PETR3.SA","PETR4.SA","PRIO3.SA","RADL3.SA","RAIL3.SA","RAIZ4.SA","RENT3.SA",
    "RECV3.SA","SANB11.SA","SBSP3.SA","SLCE3.SA","SMTO3.SA","SUZB3.SA","TAEE11.SA","TIMS3.SA",
    "TTEN3.SA","TOTS3.SA","TRPL4.SA",
    "UGPA3.SA","USIM5.SA","VALE3.SA","VIVT3.SA","VIVA3.SA","WEGE3.SA","YDUQ3.SA","AURE3.SA","BHIA3.SA","CASH3.SA",
    "CVCB3.SA","DIRR3.SA","ENAT3.SA","GMAT3.SA","IFCM3.SA","INTB3.SA","JHSF3.SA","KEPL3.SA","MOVI3.SA","ORVR3.SA",
    "PETZ3.SA","PLAS3.SA","POMO4.SA","POSI3.SA","RANI3.SA","RAPT4.SA","STBP3.SA","TEND3.SA","TUPY3.SA",
    "BRSR6.SA","CXSE3.SA","AAPL34.SA","AMZO34.SA","GOGL34.SA","MSFT34.SA","TSLA34.SA","META34.SA","NFLX34.SA",
    "NVDC34.SA","MELI34.SA","BABA34.SA","DISB34.SA","PYPL34.SA","JNJB34.SA","PGCO34.SA","KOCH34.SA","VISA34.SA",
    "WMTB34.SA","NIKE34.SA","ADBE34.SA","AVGO34.SA","CSCO34.SA","COST34.SA","CVSH34.SA","GECO34.SA","GSGI34.SA",
    "HDCO34.SA","INTC34.SA","JPMC34.SA","MAEL34.SA","MCDP34.SA","MDLZ34.SA","MRCK34.SA","ORCL34.SA","PEP334.SA",
    "PFIZ34.SA","PMIC34.SA","QCOM34.SA","SBUX34.SA","TGTB34.SA","TMOS34.SA","TXN34.SA","UNHH34.SA","UPSB34.SA",
    "VZUA34.SA","ABTT34.SA","AMGN34.SA","AXPB34.SA","BAOO34.SA","CATP34.SA","HONB34.SA","BOVA11.SA","IVVB11.SA",
    "SMAL11.SA","HASH11.SA","GOLD11.SA","GARE11.SA","HGLG11.SA","XPLG11.SA","VILG11.SA","BRCO11.SA","BTLG11.SA",
    "XPML11.SA","VISC11.SA","HSML11.SA","MALL11.SA","KNRI11.SA","JSRE11.SA","PVBI11.SA","HGRE11.SA","MXRF11.SA",
    "KNCR11.SA","KNIP11.SA","CPTS11.SA","IRDM11.SA","DIVO11.SA","NDIV11.SA","SPUB11.SA"
]))

# =========================================================
# CONFIGURAÇÕES DE ALVOS (BUY SIDE)
# =========================================================
STOPS_CONFIG = {
    'ACAO': {'gain': 0.08, 'loss': -0.05},
    'BDR':  {'gain': 0.06, 'loss': -0.04},
    'ETF':  {'gain': 0.05, 'loss': -0.03}
}

# =========================================================
# Funções auxiliares
# =========================================================

def identificar_tipo(ticker):
    if '34.SA' in ticker: return 'BDR'
    if '11.SA' in ticker: return 'ETF'
    return 'ACAO'

def calcular_probabilidade_ganho(df_hist, tipo_ativo):
    """Calcula a probabilidade histórica do ativo bater o Gain antes do Loss."""
    params = STOPS_CONFIG[tipo_ativo]
    gain_target = params['gain']
    loss_target = params['loss']
    
    sucessos = 0
    tentativas = 0
    
    # Analisa janelas nos últimos 250 dias (aprox 1 ano) para maior relevância
    df_prob = df_hist.tail(250).copy()
    
    for i in range(0, len(df_prob) - 20, 3):
        preco_entrada = df_prob['Close'].iloc[i]
        gain_price = preco_entrada * (1 + gain_target)
        loss_price = preco_entrada * (1 + loss_target)
        
        tentativas += 1
        for j in range(i + 1, min(i + 22, len(df_prob))):
            if df_prob['High'].iloc[j] >= gain_price:
                sucessos += 1
                break
            if df_prob['Low'].iloc[j] <= loss_price:
                break
                
    return (sucessos / tentativas * 100) if tentativas > 0 else 0

def ajustar_colunas(df):
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(0)
    for col in df.columns:
        if isinstance(df[col], pd.DataFrame):
            df[col] = df[col].iloc[:, 0]
    return df

def ema(series, period):
    return series.ewm(span=period, adjust=False).mean()

def rma(series, period):
    return series.ewm(alpha=1/period, adjust=False).mean()

def stochastic_kd(df, k_period=14, d_period=3, smooth=3):
    low_min = df["Low"].rolling(k_period).min()
    high_max = df["High"].rolling(k_period).max()
    k = 100 * (df["Close"] - low_min) / (high_max - low_min)
    k_smooth = k.rolling(smooth).mean()
    d = k_smooth.rolling(d_period).mean()
    return k_smooth, d

def dmi_adx_tradingview(df, period=14):
    high, low, close = df["High"], df["Low"], df["Close"]
    up, down = high.diff(), -low.diff()
    plus_dm = np.where((up > down) & (up > 0), up, 0.0)
    minus_dm = np.where((down > up) & (down > 0), down, 0.0)
    tr = pd.concat([(high - low), (high - close.shift()).abs(), (low - close.shift()).abs()], axis=1).max(axis=1)
    tr_rma = rma(tr, period)
    plus_di = 100 * rma(pd.Series(plus_dm, index=df.index), period) / tr_rma
    minus_di = 100 * rma(pd.Series(minus_dm, index=df.index), period) / tr_rma
    adx = rma(100 * (abs(plus_di - minus_di) / (plus_di + minus_di)), period)
    return plus_di, minus_di, adx

def preparar_semanal(df):
    return df.resample("W-FRI").agg({"Open": "first", "High": "max", "Low": "min", "Close": "last", "Volume": "sum"})

def indice_candle_fechado():
    tz = pytz.timezone("America/Sao_Paulo")
    agora = datetime.now(tz).time()
    # Horários solicitados: 19:00 e 19:15.
    # O candle do dia é considerado fechado para análise após as 19h.
    if agora >= time(19, 0):
        return -1
    else:
        return -2

# =========================================================
# Scanner
# =========================================================

if st.button("Rodar Scanner"):
    resultados = []
    progress = st.progress(0.0)
    idx = indice_candle_fechado()

    for i, ticker in enumerate(ativos_scan):
        try:
            df = yf.download(ticker, period="450d", interval="1d", progress=False, auto_adjust=False)
            if df.empty or len(df) < 120:
                continue

            df = ajustar_colunas(df)
            tipo_at = identificar_tipo(ticker)

            # --- Cálculos Diário ---
            df["EMA69"] = ema(df["Close"], 69)
            k, d = stochastic_kd(df)
            df["K"], df["D"] = k, d
            di_p, di_m, adx = dmi_adx_tradingview(df)
            df["DIp"], df["DIm"], df["ADX"] = di_p, di_m, adx
            df["Vol_MA20"] = df["Volume"].rolling(20).mean()
            df["Vol_MA50"] = df["Volume"].rolling(50).mean()

            if len(df.dropna()) < abs(idx): continue
            row = df.iloc[idx]

            # Filtros Diário
            cond_ema = row["Close"] > row["EMA69"]
            cond_stoch = row["K"] > row["D"]
            cond_dmi = row["DIp"] > row["DIm"]
            cond_vol = row["Vol_MA20"] > row["Vol_MA50"]

            if not (cond_ema and cond_stoch and cond_dmi and cond_vol):
                continue

            # --- Cálculos Semanal ---
            semanal = preparar_semanal(df)
            semanal = ajustar_colunas(semanal)
            kw, dw = stochastic_kd(semanal)
            di_pw, di_mw, _ = dmi_adx_tradingview(semanal)
            
            if len(semanal.dropna()) < abs(idx): continue
            row_w = semanal.iloc[idx]

            cond_sem_dmi = row_w["DIp"] > row_w["DIm"]
            cond_sem_stoch = kw.iloc[idx] > dw.iloc[idx]

            if not (cond_sem_dmi and cond_sem_stoch):
                continue

            # --- Cálculo de Probabilidade Estatística (Novo) ---
            prob_sucesso = calcular_probabilidade_ganho(df, tipo_at)

            resultados.append({
                "Ativo": ticker,
                "Classe": tipo_at,
                "Data": df.index[idx].date(),
                "Close": round(float(row["Close"]), 2),
                "Prob. Gain (%)": round(prob_sucesso, 2),
                "Gain Alvo": f"{STOPS_CONFIG[tipo_at]['gain']*100}%",
                "Stop Loss": f"{STOPS_CONFIG[tipo_at]['loss']*100}%",
                "ADX (D)": round(float(row["ADX"]), 2),
                "K (W)": round(float(kw.iloc[idx]), 2),
                "D (W)": round(float(dw.iloc[idx]), 2),
                "Vol MA20 > MA50": cond_vol
            })

        except Exception:
            pass
        progress.progress((i + 1) / len(ativos_scan))

    st.subheader("Ativos aprovados no setup")

    if len(resultados) == 0:
        st.warning("Nenhum ativo passou em todos os filtros.")
    else:
        df_res = pd.DataFrame(resultados).sort_values("Prob. Gain (%)", ascending=False)
        
        # Estilização para destacar a probabilidade
        st.dataframe(df_res.style.highlight_max(axis=0, subset=['Prob. Gain (%)'], color='#2E7D32'), use_container_width=True)
        
        st.success(f"Scanner finalizado. {len(df_res)} ativos encontrados.")

# Rodapé informativo
st.sidebar.info(f"Horário de Brasília: {datetime.now(pytz.timezone('America/Sao_Paulo')).strftime('%H:%M:%S')}")
